...
class FileManager:
    ...
    def _save_file(self,doc_name: str, doc_string: str | None=None, overwrite=False):
        ...
        if settings.LOG_MSG: log.debug(f'WRITE MODE:{write_mode}') #^DEBUG
        
        try: # CREATE FILE
            with open(doc_name, write_mode, encoding='utf-8')as f:
                if doc_string is not None: # WRITE DOC_STRING TO FILE
                    if settings.LOG_MSG: log.watchdog(f'WRITING DOCSTRING: {doc_name}') #^DEBUG
                    f.write(doc_string)
                    f.close()
            ...
            if settings.LOG_MSG: log.warning(f'FILE ALREADY EXISTS: {doc_name}!') #^DEBUG

    #? CREATE A NEW FOLDER OR FOLDER TREE FROM PATH
    def _create_folder(self, folder_path: str,exist_ok=True):
        os.makedirs(folder_path, exist_ok=exist_ok)
        if settings.LOG_MSG: log.watchdog(f'CREATED NEW FOLDER PATH: {folder_path}')
    ...
    def _create_if_missing(self, fname, path=None) -> str:
        ...
        # SEARCH RECURSIVELY
        if settings.LOG_MSG: log.warning(f'{new_path} NOT FOUND; SEARCHING {root}.')
        for file in root.rglob(fname):
            if file.is_file():
                if settings.LOG_MSG: log.debug(f'FOUND FILE: {file}') #^DEBUG
                return str(file) #!EXIT!
        
        # STILL NOT FOUND; CREATE NEW
        if settings.LOG_MSG: log.warning(f'{new_path}: STILL NOT FOUND, CREATING...') #^DEBUG
        try:
            new_path.write_text(f"{fname} | CREATED: {time.time()}", encoding='utf-8')
        except FileExistsError:
            if settings.LOG_MSG: log.critical(f'{fname} FOUND AT: {new_path}') #^DEBUG
        return str(new_path) #!EXIT!    
    ...
    def __setattr__(self, name: str, value: str) -> None:
        if 'path' in name:
            ...
            if settings.LOG_MSG: log.info(f'UPDATING {name} -> {value}') #^DEBUG
    ...
    def cur_find_file(self, filename:str) -> str | None:
        ...
        if settings.LOG_MSG: log.warning(f'SEARCHING {root} for {filename}')
...
def test():
    with FileManager() as fm:
        #fm.get_subdirs() #? LIST VISIBLE SUB-DIRECTORIES
        fm._create_folder('./test_johnfm')
        pass        
